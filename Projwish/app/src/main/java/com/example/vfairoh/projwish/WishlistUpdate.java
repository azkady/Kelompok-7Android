...
public class WishlistUpdate extends AppCompatActivity
        implements NavigationView.OnNavigationItemSelectedListener, AdapterView.OnItemSelectedListener {

    //    ini codingan buat Add Trans
    protected Cursor cursor;
    private EditText update_detail, update_jumlah, update_id;
    private Button btn_updatewL;
    DataHelper dbHelper;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_wishlist_update);
        //UANG MASUK
        ...
        String idwL = obj.getStringExtra("idwL");
        update_id = findViewById(R.id.update_idwL);
        update_detail = findViewById(R.id.update_wdetail);
        update_jumlah = findViewById(R.id.update_wjumlah);
        btn_updatewL = findViewById(R.id.btn_updatewL);
        update_id.setEnabled(false);
        dbHelper = new DataHelper(this);
        final SQLiteDatabase db = dbHelper.getWritableDatabase();
        String sql = "select * from WishList where idwL = " + idwL;
        Cursor c = db.rawQuery(sql, null);
        if (c != null) {
            if (c.moveToFirst()) {
                int id = c.getInt(c.getColumnIndex("idwL"));
        ...
        btn_updatewL.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                db.execSQL("update WishList set detail = '"+ detail +"', jumlahUang = "+ jumlah +" where idwL = "+ id);
...
//    end codingan add Trans
        ...
        TextView txt_uangB = (TextView) navigationView.getHeaderView(0).findViewById(R.id.uangF);
